...
{
	float distance = fabsf(a *p.x + b*p.y -c);
	return distance; 
}
...
void separate_points( t_point *points, int num_point, t_point from, t_point to, t_point *s1, t_point *s2, int *n1, int *n2)
{
	float a = to.y - from.y;
	float b = from.x - to.x;
	float c = (to.y-from.y) * from.x - (to.x-from.x) * from.y;
	*n1 = 0;
	*n2 = 0;
...
t_line *upper_hull( t_point *points, int num_point, t_point p1, t_point pn, t_line *lines, int *num_line, int *capacity)
{
	...
	if(num_point ==0){
		if( *num_line >= *capacity ){
			*capacity = *capacity + 10;
			lines = realloc(lines, sizeof(t_line) * (*capacity) );
		}
	...
	//p1과 pn을 잊는 직선 중 가장 먼 점 찾기
	int max_point =0;
	float max_distance = 0;
	float a = pn.y - p1.y;
	float b = p1.x - pn.x;
	float c = (pn.y-p1.y)*p1.x - (pn.x-p1.x)*p1.y;

	for(int i = 0; i<num_point ;  i++ ){
		float point_distance = distance(a,b,c,points[i]);
	...
	// 가장 먼 점을 기준으로 왼쪽 직선 만들기
	int left_num;
	int right_num;
	t_point *left_points = (t_point *)malloc( sizeof(t_point) * num_point);
	t_point *right_points = (t_point *)malloc( sizeof(t_point) * num_point);

	separate_points(points, num_point, p1, points[max_point],left_points,right_points,&left_num,&right_num);
	lines= upper_hull(left_points,left_num,p1,points[max_point],lines,num_line,capacity);


	separate_points(points, num_point, points[max_point],pn,left_points,right_points,&left_num,&right_num);
	lines = upper_hull(left_points,left_num,points[max_point],pn,lines,num_line,capacity);
